...
int main(void)
{
	...
	printk("Gzll ack payload host example started.");
	...
	if (err) {
		...
		printk("Cannot initialize buttons (err: %d)", err);
	...
	if (!result_value) {
		LOG_ERR("Cannot initialize GZLL glue code");
				printk("Cannot initialize GZLL glue code");
